...
class TodoService implements TodoServiceProps {
  ...
  apiPaths: Record<pageModel, { [key: string]: Function }> = {
    ...
    order: {
      // 产品页面确实 不知道怎么写
      approveList: API.order.getAllApproval,
      applyList: API.order.getAllApply,
      retract: API.order.cancelJoin,
      approve: API.order.joinSuccess,
      refuse: API.order.joinRefuse,
    },
    market: {
  ...
  public retractApply = async (id: string) => {
    return await this.opretionFn(this.apiPaths[this.currentModel].retract, { id });
  };
}
